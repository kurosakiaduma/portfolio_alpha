...
set -l PORTFOLIO_DIR "/home/portfolio/frontend"
set -l PHYLO_ROBOTS_DIR "/var/www/phylo/apps/frontend/family-tree/public"
set -l NGINX_CONF "/etc/nginx/sites-available/taduma.conf"
...
if test -f "$SCRIPT_DIR/phylo-robots.txt"
...
if test -f "$SCRIPT_DIR/taduma.conf"
    # Create backup with timestamp
    set -l BACKUP_FILE "$NGINX_CONF.backup-(date +%Y%m%d-%H%M%S)"
...
if systemctl is-active --quiet portfolio-frontend
...
else if test -f "$PORTFOLIO_DIR/docker-compose.yml"
...
if curl -sf https://taduma.me/robots.txt > /dev/null
...
if curl -sf https://taduma.me/phylo/robots.txt > /dev/null
...
if curl -sf https://taduma.me/sitemap.xml > /dev/null
...
if curl -sf -o /dev/null https://taduma.me/
...
if curl -sf -o /dev/null https://taduma.me/phylo/
